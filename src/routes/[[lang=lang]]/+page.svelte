<script lang="ts">
  ...
  let filteredResults = $derived(
    searchQuery.trim() === "" 
      ? [] 
      : allTools.filter(t => 
          t.name[lang as Language].toLowerCase().includes(searchQuery.toLowerCase()) || 
          t.desc[lang as Language].toLowerCase().includes(searchQuery.toLowerCase())
        )
  );
...
<div class="max-w-[980px] mx-auto px-4 md:px-0 py-10">
  ...
  <section class="text-left mb-16">
    <h1 class="text-3xl md:text-4xl font-bold tracking-tight text-[#1a1a1a]">
      ...
      <br>
      <span class="bg-black text-white px-2 inline-block mt-1 font-mono text-2xl tracking-tighter">{ui.home.span[lang as Language]}</span>
    </h1>
    ...
    <div class="mt-10 max-w-md relative">
      ...
      {#if isFocused && searchQuery.length > 0}
        <div class="absolute top-full left-0 right-0 mt-1 bg-white border border-slate-200 shadow-xl z-50 text-left" transition:fade={{ duration: 50 }}>
          {#if filteredResults.length > 0}
            <div class="max-h-[300px] overflow-y-auto custom-scrollbar">
              {#each filteredResults as tool}
                <a href="{langPreUrl}{tool.href}" class="block px-3 py-2 hover:bg-slate-50 border-b border-slate-50 last:border-0 group">
                  <div class="flex justify-between items-center">
                    <div>
                      <span class="text-[12px] font-bold block">{tool.name[lang as Language]}</span>
                      <p class="text-[10px] text-slate-400 uppercase font-mono">{tool.desc[lang as Language]}</p>
                    </div>
          ...
          {:else}
            <div class="p-4 text-center text-[10px] font-mono text-slate-400">{ui.home.no_tool_found[lang as Language]}</div>
          {/if}
        </div>
      {/if}
    </div>
      ...
      <div class="flex flex-col group/cat">
        ...
        <ul class="space-y-4">
          {#each category.tools as tool}
            <li class="group/item">
              <a href="{langPreUrl}{tool.href}" class="block border-l-2 border-transparent hover:border-slate-200 pl-3 transition-all">
